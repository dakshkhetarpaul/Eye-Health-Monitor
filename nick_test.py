...
def play_sound(alert_type):
    """Reliable cross-platform sound alerts using sounddevice"""
    ...
    # Sound parameters for different alerts
    sounds = {
        "bad": (440, 0.8),    # Low pitch, long duration
        "good": (880, 0.3),   # High pitch, short duration
        "blink": (660, 0.5)   # Medium pitch
    }
...
while True:
    ...
    for face in faces:
        ...
        # Detect new blinks (transition from open to closed)
    ...
    # Remove blinks older than our monitoring window
    blink_timestamps = [t for t in blink_timestamps if current_time - t <= MONITOR_WINDOW]
    
    # Check for infrequent blinking (less than 3 blinks in 10s)
    if (len(blink_timestamps) < MIN_BLINKS and 
        current_time - last_alert_time > alert_cooldown and
        current_time > MONITOR_WINDOW):  # Wait until we have full window data
